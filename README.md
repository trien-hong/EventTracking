...
One in the EventTracking folder (where docker-compose.yml is located), one in django_backend folder (where Dockerfile is located), and one in react_frontend folder (where another Dockerfile is located). A combined total of 14 environment variables are needed.
